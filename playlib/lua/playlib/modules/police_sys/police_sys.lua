...
if SERVER then
	...
	PLAYLIB.Logger:Create("police_sys")

 	PLAYLIB.police_sys.log = function(text) PLAYLIB.Logger:Log("police_sys",text) end
...
function PLAYLIB.police_sys.openUI(ply)
	...
	if PLAYLIB.police_sys.canView(ply) then
		...
		self.log(ply:Name().."("..ply:SteamID64()..") tried to access the Police Panel without having permission!\n")
...
function PLAYLIB.police_sys.InitializeDatabase()
	...
	PLAYLIB.sql.query(false,queryString, function(res)
		PLAYLIB.police_sys.log("Erstellen der Datenbank erfolgreich!") 
	...
	function(error) 
		...
		PLAYLIB.police_sys.log("Beim erstellen des SQLite Tables ist ein Error aufgetreten!")
		PLAYLIB.police_sys.log("Error: "..error)
	end)
end
...
function PLAYLIB.police_sys.getPoliceEntries()
	...
	function(error) 
		...
		MsgC(Color(255,255,255),"[",PLAYLIB.police_sys.PrefixColor,PLAYLIB.police_sys.Prefix,Color(255,255,255),"] [POLICE SYS] Error: "..error.."\n")
		self.log("Beim erfragen der Police Einträge ist ein Error aufgetreten!")
