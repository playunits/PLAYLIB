SWEP.PrintName = "Passport PoliceRP"
 
SWEP.Author = "Kerry | playunits"
SWEP.Contact = "just add on Steam"
SWEP.Purpose = "Description"
SWEP.Instructions = "Kerry | playunits"
		...
		if (m) then
			...
			cam.Start3D2D(pos+ang:Right()*3+ang:Forward()*-1.80+ang:Up()*1.0, ang, 0.02)
			        ...
			        if self.Owner:isCP() then
			        	name = string.sub(self.Owner:getDarkRPVar("rpname"),3)
			        else
			        	name = self.Owner:getDarkRPVar("rpname")
			        end
					draw.SimpleText(name or "Unknown", "PassHud", 40, -40, Color(0,0,0,255))
					draw.SimpleText("Job:", "PassHud2", 40, -17, Color(0,0,0,255))
					local job = self.Owner:getDarkRPVar("job")
					if #job > 20 then
						job = string.sub(job,17,#job).."..."
					end
					draw.SimpleText(job or "Unknown", "PassHud3", 40, -5, Color(0,0,0,255))
					draw.SimpleText("City:", "PassHud2", 40, 15, Color(0,0,0,255))
					draw.SimpleText("Miami", "PassHud3", 40, 30, Color(0,0,0,255))
